...
export const createQuestionnare =
	(answersArray, step, completed, history, ENDPOINT = '/api/create_questionnaire') =>
	async (dispatch) => {
		try {
			let answers = [];
...
export const save_basic_details =
	(body, url, cityName, availNowResponseText, ENDPOINT = '/api/save_basic_details') =>
	async (dispatch) => {
		try {
			const { lead } = await _post(ENDPOINT, body);
			dispatch(set_user_details(lead));
...
export const downloadReport = (downloadText, setDownloadText) => {
	...
	const url = `${process.env.REACT_APP_BACKEND_URL}/api/download_report?lead_id=` + localStorage.getItem('lead_id');
	axios
		.request({
			url,
			method,
...
export const emailReport = () => async (dispatch) => {
	try {
		const { message } = await _post('/api/email_report?lead_id=' + localStorage.getItem('lead_id'));
